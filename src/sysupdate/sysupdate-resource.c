...
static int process_magic_file(
        ...
        struct tm copy_tm = parsed_tm; // Make a copy to avoid modifying the original
        usec_t best_before;
        r = mktime_or_timegm_usec(&copy_tm, true, &best_before);
        if (r < 0)
        ...
        || copy_tm.tm_year != parsed_tm.tm_year) {
                log_warning("Found best before marker with a non-normalized date, ignoring: %s", fn);
                return 0;
        }
